...
namespace Gymany.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        private string apiGetAdminByID;
        ...
        public AdminController()
        {
            ...
            this.api_Admin = "https://localhost:5002/api/Admin/checklogin";
            this.apiGetAdminByID = "https://localhost:5002/api/Admin";
        ...
        public async Task<IActionResult> Profile(string error = null)
        {
            ...
            if (HttpContext.Session.GetString("Role").Equals("Admin"))
            {
                ...
                apiGetAdminByID = $"https://localhost:5002/api/Admin/id?id={id}";
                HttpResponseMessage response = await client.GetAsync(apiGetAdminByID);
                string data = await response.Content.ReadAsStringAsync();
                var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
                Admin Admin = JsonSerializer.Deserialize<Admin>(data, options);
                return View(Admin);
            }else{
                ViewBag.Name = HttpContext.Session.GetString("AdminName");
                string id = HttpContext.Session.GetString("AdminID");
        ...
        [HttpPost]
        public async Task<IActionResult> Profile(Admin obj)
        {
            if (HttpContext.Session.GetString("Role").Equals("Admin"))
            {
                ViewBag.Name = HttpContext.Session.GetString("AdminName");
                ...
                HttpResponseMessage response = await client.PutAsync(apiGetAdminByID, content);
                if (response.IsSuccessStatusCode)
                {
                    HttpContext.Session.SetString("AdminName", obj.admin_name.ToString());
        ...
        {
            api_Admin = $"https://localhost:5002/api/Admin/checklogin?username={username}&password={password}";
            var Admin = new Admin { admin_username = username, admin_password = password };
            var content = new StringContent(JsonSerializer.Serialize(Admin), Encoding.UTF8, "application/json");
            HttpResponseMessage response = await client.PostAsync(api_Admin, content);
            string data = await response.Content.ReadAsStringAsync();
            var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
            Admin gym = JsonSerializer.Deserialize<Admin>(data, options);
            if (response.IsSuccessStatusCode)
            {
                // var user = await response.Content.ReadFromJsonAsync<Admin>();
                // Lưu thông tin người dùng vào session hoặc cookie
                HttpContext.Session.SetString("UsernameAdmin", username);
                HttpContext.Session.SetString("PasswordAdmin", password);
                HttpContext.Session.SetString("AdminName", gym.admin_name.ToString());
                HttpContext.Session.SetString("AdminID", gym.admin_id.ToString());
            ...
            {
                api_Admin = $"https://localhost:5002/api/Admin/checklogin?username={username}&password={password}";
                var staff = new Admin { admin_username = username, admin_password = password };
                var contentAdmin = new StringContent(JsonSerializer.Serialize(staff), Encoding.UTF8, "application/json");
                HttpResponseMessage responseStaff = await client.PostAsync(api_Admin, contentAdmin);
                ...
                Admin staffnew = JsonSerializer.Deserialize<Admin>(dataStaff, optionsStaff);
                if (responseStaff.IsSuccessStatusCode)
                {
                    HttpContext.Session.SetString("UsernameAdmin", username);
                    HttpContext.Session.SetString("PasswordAdmin", password);
                    HttpContext.Session.SetString("AdminName", staffnew.admin_name.ToString());
                    HttpContext.Session.SetString("AdminID", staffnew.admin_id.ToString());
       ...
       // ------------------------------------Order Manage --------------------------------\\
        public async Task<IActionResult> Order(int? page)
        {
            ...
            if(HttpContext.Session.GetString("Role").Equals("Admin")){
                ...
                var listPage = list.ToPagedList(page ?? 1, 6);
                return View(listPage);
            } 
        ...
        public async Task<List<SelectListItem>> GetStaffId()
        {
            ...
            string data = await respone.Content.ReadAsStringAsync();
            var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
            List<Admin> list = JsonSerializer.Deserialize<List<Admin>>(data, options);
            List<SelectListItem> yourData = list.Select(c => new SelectListItem
            {
                Value = c.admin_id.ToString(), // ID của category là giá trị của mục
                Text = c.admin_name // Tên của category là nội dung của mục
            }).ToList();
            return yourData;
        }
        ...
        public async Task<List<SelectListItem>> GetCustomerId()
        {
            ...
            List<SelectListItem> yourData = list.Select(c => new SelectListItem
            {
                Value = c.cus_id.ToString(), // ID của category là giá trị của mục
                Text = c.cus_name                                // Tên của category là nội dung của mục
            }).ToList();
            return yourData;
        }
        public async Task<List<SelectListItem>> GetProduct()
        {
            ...
            List<SelectListItem> yourData = list.Select(c => new SelectListItem
            {
                Value = c.prod_id.ToString(), // ID của category là giá trị của mục
                Text = c.prod_name                                // Tên của category là nội dung của mục
            }).ToList();
            return yourData;
        }
        ...
        {
            var email = HttpContext.Session.GetString("UsernameAdmin");
            var pass = HttpContext.Session.GetString("PasswordAdmin");
